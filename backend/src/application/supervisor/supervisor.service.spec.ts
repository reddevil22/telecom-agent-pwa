...
function mockToolCall(
  name: string,
  args: Record<string, unknown> = { userId: "user-42" },
  id = "call-1",
): LlmChatResponse {
  return {
    message: {
...
function createMockSubAgent(screenType: string): SubAgentPort {
  return {
    handle: jest.fn().mockImplementation((userId: string) =>
      Promise.resolve({
...
describe("SupervisorService", () => {
  ...
  it("temporarily disables a tool after repeated sub-agent failures", async () => {
    ...
    const fourthLlmInput = (mockLlm.chatCompletion as jest.Mock).mock
      .calls[3]?.[0] as {
      tools?: Array<{ function: { name: string } }>;
    };
    expect(
      fourthLlmInput.tools?.some(
        (tool) => tool.function.name === "check_balance",
      ),
    ).toBe(false);
  });
  ...
  it("scopes temporary tool disable per user", async () => {
    ...
    service.registerAgent("check_balance", failingAgent);
    (mockLlm.chatCompletion as jest.Mock).mockResolvedValue(
      mockToolCall("check_balance"),
    );

    await collectResult(
      service.processRequest(makeRequest({ userId: "user-a" })),
    );
    await collectResult(
      service.processRequest(makeRequest({ userId: "user-a" })),
    );
    ...
    const blockedForUserA = await collectResult(
      service.processRequest(makeRequest({ userId: "user-a" })),
    );
    const firstFailureForUserB = await collectResult(
      service.processRequest(makeRequest({ userId: "user-b" })),
    );

    expect(blockedForUserA.errorCode).toBe(
  ...
  it("re-enables temporarily disabled tools after cooldown", async () => {
    ...
    nowSpy.mockReturnValue(
      Date.now() + SECURITY_LIMITS.SUB_AGENT_DISABLE_MS + 1,
    );

    const afterCooldown = await collectResult(
      service.processRequest(makeRequest()),
    );
  ...
  it("returns unknown with LLM text as replyText when LLM gives no tool call", async () => {
    (mockLlm.chatCompletion as jest.Mock).mockResolvedValueOnce(
      mockTextResponse("Could you rephrase your question?"),
    );
  ...
  it("feeds error message back for tool call with unexpected arguments", async () => {
    ...
    const secondCallMessages = (mockLlm.chatCompletion as jest.Mock).mock
      .calls[1][0].messages;
  ...
  it("returns unknown when all iterations produce invalid tool calls", async () => {
    ...
    expect(mockLlm.chatCompletion).toHaveBeenCalledTimes(
      SECURITY_LIMITS.SUPERVISOR_MAX_ITERATIONS,
    );
  });
  ...
  it("returns error response when LLM throws", async () => {
    (mockLlm.chatCompletion as jest.Mock).mockRejectedValueOnce(
      new Error("LLM down"),
    );
  ...
  it("handles LLM returning text content alongside tool calls", async () => {
    ...
    const serviceWithLogger = new SupervisorService(
      mockLlm,
      "test-model",
      0.1,
      1024,
        ...
        tool_calls: [
          {
            ...
            function: {
              name: "check_balance",
              arguments: JSON.stringify({ userId: "u1" }),
            },
          },
        ],
      },
    ...
    const result = await collectResult(
      serviceWithLogger.processRequest(makeRequest()),
    );
    expect(result.screenType).toBe("balance");
  ...
  it("caps conversation history to SUPERVISOR_HISTORY_CAP", async () => {
    ...
    await collectResult(
      service.processRequest(makeRequest({ conversationHistory: longHistory })),
    );

    const callArgs = (mockLlm.chatCompletion as jest.Mock).mock.calls[0][0];
    // System prompt + capped history + current user message
    expect(callArgs.messages.length).toBe(
      SECURITY_LIMITS.SUPERVISOR_HISTORY_CAP + 2,
    );
  });
  ...
  it("trims oldest messages when total chars exceed budget", async () => {
    ...
    await collectResult(
      service.processRequest(makeRequest({ conversationHistory: hugeHistory })),
    );

    const callArgs = (mockLlm.chatCompletion as jest.Mock).mock.calls[0][0];
    const totalChars = callArgs.messages.reduce(
      (sum: number, m: { content: string }) => sum + m.content.length,
      0,
    );
    expect(totalChars).toBeLessThanOrEqual(
      SECURITY_LIMITS.TOTAL_CHARS_BUDGET + 100,
    );
  });
  ...
  describe("screen caching", () => {
    ...
    beforeEach(() => {
      ...
      cachedService = new SupervisorService(
        mockLlm,
        "test-model",
        0.1,
        1024,
    ...
    it("returns cached response on second balance query without calling LLM", async () => {
      ...
      const first = await collectResult(
        cachedService.processRequest(
          makeRequest({ prompt: "Show my balance" }),
        ),
      );
      ...
      const second = await collectResult(
        cachedService.processRequest(
          makeRequest({ prompt: "What is my balance?" }),
        ),
      );
    ...
    it("invalidates only impacted screens after confirmation screen", async () => {
      ...
      await collectResult(
        cachedService.processRequest(makeRequest({ prompt: "I need support" })),
      );
      expect(mockLlm.chatCompletion).toHaveBeenCalledTimes(2);
      ...
      (mockLlm.chatCompletion as jest.Mock).mockResolvedValueOnce(
        mockToolCall("purchase_bundle"),
      );
      await collectResult(
        cachedService.processRequest(
          makeRequest({ prompt: "Buy the data bundle" }),
        ),
      );
      ...
      const balanceResult = await collectResult(
        cachedService.processRequest(
          makeRequest({ prompt: "Show my balance" }),
        ),
      );
      ...
      const supportResult = await collectResult(
        cachedService.processRequest(makeRequest({ prompt: "I need support" })),
      );
      expect(supportResult.screenType).toBe("support");
      expect(supportResult.processingSteps[0].label).toBe(
