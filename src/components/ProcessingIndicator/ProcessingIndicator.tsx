...
export function ProcessingIndicator({ steps }: Props) {
  ...
  if (steps.length > 0) {
    ...
    const currentIndex = activeIndex >= 0 ? activeIndex : Math.max(steps.length - 1, 0);
    const currentStep = steps[currentIndex];
    const summary = `Step ${currentIndex + 1} of ${steps.length}: ${currentStep.label} (${toStatusLabel(currentStep.status)})`;

    return (
      <div className={styles.indicator} role="status" aria-live="polite" aria-label="Processing your request">
